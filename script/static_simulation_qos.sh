...
sudo tc class add dev $GW_IFACE parent 1:1 classid 1:20 htb rate $UP_REST prio 3
...
sudo tc filter add dev $GW_IFACE protocol ip parent 1:0 prio 3 handle 10 fw flowid 1:20
...
sudo tc class add dev $ST_IFACE parent 1:1 classid 1:20 htb rate $RET_REST prio 3
...
sudo tc filter add dev $ST_IFACE protocol ip parent 1:0 prio 3 handle 20 fw flowid 1:20
